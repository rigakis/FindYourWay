...
<html lang="el-GR">
  <head>
    ...
    <meta name="description" content="Αρχική σελίδα find your way.">
...
<div class="mdl-layout mdl-js-layout mdl-layout--fixed-header">
	...
	<main class="mdl-layout__content">
		<div class="mdl-grid">
		  <div class="results-column no-margin">
        <div class="results"></div>
      </div>
		  
		  <div class="map-column no-margin">
			<div class="map-wrap">	
				<div id="map"></div>
...
function createSearchData(){
    ...
    var insert_ratings = document.getElementsByClassName('insert-rating');
    var ratings = document.getElementsByClassName('rating r1');
    ...
    for (var i = 0; i < companies.length; i++) {
			...
			search_data_element={
                ...
                insert_rating:insert_ratings[i].id,
                type:types[i].innerHTML,
                city:cities[i].innerHTML,
                favorite:favorite,
                rating:ratings[i].dataset.width,
...
function addMarkersToMap(){
		...
		google.maps.event.addListener(marker, 'click', (function(marker, search_data_element) {
			return function() {
			  ...
			  "<a href='"+search_data_element.detail+"'>Λεπτομέριες</a>");
...
// Adds a marker to the map and push to the array.
    ...
    function SortSearchDataCompanyRatingDesc(){
        search_data.sort(function(a, b) {
    ...
    function SortSearchDataCompanyRatingAsc(){
        search_data.sort(function(a, b) {
...
function addDistances(travelmode){
    ...
    distanceService.getDistanceMatrix({
          ...
          if (status !== 'OK') {
            ...
            var checkdistance = document.getElementById('distance');
            if (checkdistance != null) {
              checkdistance.parentElement.removeChild(checkdistance);
            }
            var checkduration = document.getElementById('duration');
            if (checkduration != null) {
              checkduration.parentElement.removeChild(checkduration);
            }
            var checkdirection = document.getElementById('btn-direction');
            if (checkdirection != null) {
              checkdirection.parentElement.removeChild(checkdirection);
            }
            ...
            for (var i = 0; i < origins.length; i++) {
                ...
                for (var j = 0; j < results.length; j++) {
                    ...
                    if(element.status=="OK"){
                        ...
                        // details[j].innerHTML+="<p id='btn-direction'><a onclick='getDirections("+search_data[j].company.lat+","+search_data[j].company.lng+")'><i class='material-icons'>directions</i> Οδηγίες πλοήγησης</a></p>";
                    }
                }
            }
...
console.log("updateSearchResultsBar");
    var html = "";
    for (var i = 0; i < search_data.length; i++) {
		var id = search_data[i].insert_rating;
    ...
    html += "<div class='rating-favorite'><div class='insert-rating' id='insert-rating-"+substr+"'>" +
                        "<div class='rating-wrap'>"+
                            "<div style='width:"+search_data[i].rating+"%' class='rating r1' data-width='"+search_data[i].rating+"'></div>"+
    ...
    if(search_data[i].favorite) html +=" <a class='favorite' id='fav-"+substr+"' href='javascript:addFav("+substr+")'><i class='material-icons'>"+search_data[i].favorite_type+"</i></a>";
      ...
      html +="<div class='details'>";
        ...
        // html +="<p><button onclick='getDirections("+search_data[i].company.lat+","+search_data[i].company.lng+")'>Get Directions</button></p>";
      html +="</div>";
    html +="</div>";

    }

    $('.companies-results').empty();
    $('.companies-results').html(html);
...
$(function() {
	...
	<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBSzQ33eKZJ9v3FuuvaxXbiLKc5_SWPEI8&callback=initMap2&libraries=places&language=el&region=GR" async defer></script>
</body>
</html>
